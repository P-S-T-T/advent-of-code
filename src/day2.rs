...
//   let parsed_pw_list = parse_input_pw_list(&password_list).unwrap();

//   println!("Answer of Part 1 is: {}", part1(&parsed_pw_list));
//   println!("Answer of Part 2 is: {}", part2(&parsed_pw_list));
// }
...
#[aoc_generator(day2)]
// fn parse_input_pw_list<'a>(password_list: &[&'a str]) -> Result<Vec<PasswdRule<'a>>, ParseError> {
// fn parse_input_pw_list<'a>(password_list: &'a str) -> Result<Vec<PasswdRule<'a>>, ParseError> {
fn parse_input_pw_list(password_list: &str) -> Vec<PasswdRule> {
  password_list
    ...
    .map(|password_check| {
      ...
      Ok(PasswdRule {
        first_number: min_occurrence,
        second_number: max_occurrence,
        rule_char,
        pw,
      })
    })
...
fn part1(parsed_rules: &[PasswdRule]) -> usize {
  parsed_rules
    ...
    .filter(|rule| {
      ...
      occurrences >= rule.first_number && occurrences <= rule.second_number
    })
    .count()
}
...
fn part2(parsed_rules: &[PasswdRule]) -> usize {
  parsed_rules
    .iter()
    .filter(|rule| {
      if rule.second_number > rule.pw.chars().count() {
        false
      } else {
        let mut pw_iter = rule.pw.chars();
        let first_char = pw_iter
          .nth(rule.first_number - 1)
          .ok_or(ParseError::NoneError);
        let second_char = pw_iter
          .nth(rule.second_number - rule.first_number - 1)
          .ok_or(ParseError::NoneError);
